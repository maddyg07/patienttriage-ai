...
class Clinic:
    ...
    def view(self, session: ClinicSession) -> dict:
        ...
        question, why = self.next_question(session)
    ...
    def next_question(self, session: ClinicSession):
        ...
        if session.model_question and "model" not in asked:
            return session.model_question, "generated by the language model from "\
                                           "this conversation"
        try:
            ...
            if value is not None and value.question.id not in asked:
                ...
                return value.question.text, (
                    f"value of information: {value.question.id}, could move the "
                    f"band by {span}")
        ...
        for qid, applies, text in self.OPENING_LADDER:
            ...
            try:
                if applies(session):
                    return text, f"opening ladder: {qid.replace('_', ' ')}"
            except Exception:                               # noqa: BLE001
                continue
        return "", ""
