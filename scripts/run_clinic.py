...
class Clinic:
    ...
    def view(self, session: ClinicSession) -> dict:
        ...
        snap["next_question"] = self.next_question(session)
        snap["notes"] = (session.nurse_notes_override
                         or self.notes.get(session.session_id)
                         or generate_notes(snap))
        return snap

    def next_question(self, session: ClinicSession) -> str:
        """
        The next question worth asking, or nothing.

        Two ways to get nothing, and they are different. The gate has fired, in
        which case asking anything is wrong; or no question can change the
        band, in which case asking is noise. Phase 11's value-of-information
        engine answers the second.
        """
        if not session.routine_questions_allowed:
            return ""
        try:
            from core.intake_bridge import build_patient
            patient = build_patient(session._payload())
            value = self.questions.next_question(patient)
            if value is None:
                return ""
            asked = {q["id"] for q in session.questions_asked}
            if value.question.id in asked:
                return ""
            return value.question.text
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, route: str, session: ClinicSession, p: dict) -> dict:
        ...
        if route == "/api/finish":
            ...
            session._log("session", "patient ended the encounter")
            session._broadcast()
            return {"ok": True}
    ...
    def _frame_measurements(self, session: ClinicSession, p: dict) -> dict:
        ...
        camera = {
            ...
            "spread": 0.01,                 # a periodic scan is a single frame
        ...
        if facial.strength in ("strong", "possible"):
            already = any(o["kind"] == "asymmetry"
                          for o in session.visual_observations)
            if not already:
                session.observe_visual(
                    "asymmetry",
                    f"Facial asymmetry candidate ({facial.strength}). "
                    f"Whether this is new or lifelong is unknown from the "
                    f"image and must be asked.",
...
def main():
    ...
    Handler.patient_page = render_patient({})
